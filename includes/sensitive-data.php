...
function sensitiveDataKey(): string {
    ...
    $raw = data_encryption_key_raw();
    if ($raw === '') {
        $raw = hash('sha256', (
            (env_value('DB_PASSWORD', 'bpp'))
            . '|'
            . (env_value('BREVO_API_KEY', 'bpp'))
            . '|bantaypurrpaws'
        ), true);
...
function encryptSensitiveValue(string $plaintext): string {
    ...
    if (!function_exists('sodium_crypto_secretbox')) {
        // Without libsodium, keep plaintext (email_hash handles secure lookup).
        return $plaintext;
    }
...
/**
 * Restore email column when it was incorrectly stored as hash:v1 (libsodium was off).
 */
function repairUserEmailIfNeeded(int $userId, array $user, string $deliverableEmail): void {
    if ($deliverableEmail === '' || $userId <= 0) {
        return;
    }

    $stored = (string) ($user['email'] ?? '');
    if (filter_var($stored, FILTER_VALIDATE_EMAIL)) {
        return;
    }
    ...
    updateUserFields($userId, protectEmail($deliverableEmail));
}
