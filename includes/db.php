<?php
// ============================================================
//  BantayPurrPaws — MySQL database layer (InfinityFree / XAMPP)
//
//  Copy .env.example → .env and set DB_* credentials.
//  Import sql/infinityfree_import.sql via phpMyAdmin.
// ============================================================

require_once dirname(__DIR__) . '/config/config.php';

if (!db_is_configured()) {
    error_log('BantayPurrPaws: MySQL credentials missing from .env');
    db_config_error_response();
...
function getDB(): PDO
{
    ...
    try {
        $pdo = new PDO($dsn, $cfg['user'], $cfg['pass'], db_pdo_options());
